...
MT_EXP = {"MT7": ["MiniGrid-DoorKey-6x6-v0", "MiniGrid-DistShift1-v0",
          ...
          "TL3_5": ["MiniGrid-DoorKey-6x6-v0", "MiniGrid-DistShift1-v0"],
          "TL5_7": ["MiniGrid-SimpleCrossingS9N2-v0", "MiniGrid-MultiRoom-N2-S4-v0"]
          }
...
def run_experiment(args, save_dir, exp_id = 0, seed = None):
    ...
    if args.load_critic:
        single_logger.info("Loading Critic Experts!")
        agent._V.model.network.load_shared_backbone(os.path.join(args.load_critic, f"seed_{exp_id}", "critic_model", "critic_backbone.pth"))
    
    if args.load_actor:
        single_logger.info("Loading Actor Experts!")
        agent.policy._logits.model.network.load_shared_backbone(os.path.join(args.load_actor, f"seed_{exp_id}", "actor_model", "actor_backbone.pth"))
...
if __name__ == '__main__':
    ...
    # logging
    results_dir = os.path.join(args.results_dir, "minigrid", "TL", args.env_name)
